...
import { apiBaseUrl, displayDate, fetchCollection } from '../api.js';
import { CollectionState } from './CollectionState.jsx';

export default function Activities() {
  const [activities, setActivities] = useState([]);
  const [state, setState] = useState({ loading: true, error: '' });
  const activitiesEndpoint = import.meta.env.VITE_CODESPACE_NAME ? `https://${import.meta.env.VITE_CODESPACE_NAME}-8000.app.github.dev/api/activities/` : `${apiBaseUrl}/activities/`;
  useEffect(() => { fetchCollection('activities', activitiesEndpoint).then(setActivities).catch(() => setState({ loading: false, error: 'Activities could not be loaded.' })).finally(() => setState((current) => ({ ...current, loading: false }))); }, [activitiesEndpoint]);
